...
강의 웹앱 전체는 Supabase Realtime Broadcast를 사용해 수강생이 원할 때만 강사 화면을 따라볼 수 있습니다. 일반 접속자는 자유롭게 자료를 보며, 상단 `강사화면 보기` 버튼을 누른 수강생만 강사의 현재 페이지, 단계, 스크롤 위치를 반영합니다. 독립 실습 HTML에도 같은 방식의 따라보기 버튼이 유지됩니다.
...
- 수강생은 메인 상단 `강사화면 보기` 버튼을 누르면 ON, 다시 누르면 OFF가 됩니다.
- 강사는 `role=host` URL로 접속하면 상단에 `강사 모드 송출 중` 표시가 나오고 현재 `pathname`, `search`, `hash`, `sectionId`, `scrollY`, `timestamp`를 700ms 이상 간격으로 broadcast 합니다.
...
5. Broadcast는 public channel로 사용하므로 개인정보를 payload에 넣지 않습니다. 현재 payload는 `room`, `pathname`, `search`, `hash`, `sectionId`, `scrollY`, `timestamp`만 보냅니다.
...
5. 수강생 화면에서 `강사화면 보기`를 누른 뒤 강사 화면에서 페이지 이동 또는 스크롤을 합니다.
